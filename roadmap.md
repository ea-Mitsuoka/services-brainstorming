...
> **加えて自社の既存資産も流用できる**：#5 は**クエリ分析ツール（構築済み）**、#6・Pack A の評価系は**IAM評価ツール（商品化中）**、#9 は**AI TF の進行中成果**を取り込めるため、本表の R&D はさらに圧縮余地がある（保守側に倒し未反映＝上振れ要因）。
...
| パック間の重複（#7 Landing Zone と単体パック） | 提案の二重・カニバリ | 「**単体＝部分導入／Landing Zone＝フル基盤**」と位置づけを明確化。重複の許容範囲を事前に定義 |
